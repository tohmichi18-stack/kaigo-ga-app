...
def s08(prs):
    ...
    card(s, IN(0.8), y, IN(5.75), IN(2.5), "費用の目安", TEAL,
         [("申立手数料：収入印紙 800円",),
          ...
          ("登記手数料：収入印紙 2,600円",),
          ("連絡用の郵便切手・診断書作成費 ほか",),
          ("鑑定費用：実施時のみ。多くは10万円以下",)], body_size=11.5)
    card(s, IN(6.78), y, IN(5.75), IN(2.5), "期間・件数", NAVY,
         [("審理期間：多くは申立てから 4か月以内",),
          ("（鑑定実施時はさらに期間を要する）",),
          ("申立件数：令和6年 約4.2万件（前年比 約+2%）",),
          ("内訳：後見開始 約2.9万 / 保佐 約0.9万 / 補助 約0.3万",),
...
def s11(prs):
    ...
    add_text(s, IN(1.0), top, IN(11.4), IN(0.62),
             ...
             anchor=MSO_ANCHOR.MIDDLE)
    # 経緯タイムライン
    add_text(s, IN(0.8), top + IN(0.78), IN(8), IN(0.3),
             [("◆ これまでの経緯", 13, TEAL, True)])
    tl = [
        ("2025.6.10", "法制審議会が中間試案を公表（パブコメ実施）"),
    ...
    y = top + IN(1.12)
    for d, t in tl:
        add_rect(s, IN(0.8), y, IN(1.7), IN(0.42), NAVY)
        add_text(s, IN(0.8), y, IN(1.7), IN(0.42), [(d, 10.5, WHITE, True)],
                 align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        add_text(s, IN(2.65), y, IN(6.0), IN(0.42), [(t, 11, INK, False)],
                 anchor=MSO_ANCHOR.MIDDLE)
        y += IN(0.5)

    add_text(s, IN(0.8), y + IN(0.08), IN(8), IN(0.3),
             [("◆ 見直しの柱", 13, TEAL, True)])
    pillars = [
        "終身制の廃止 ── 家裁の判断で制度を「終われる後見」へ",
        "利用期間制の導入 ── 必要な期間だけ利用、満了時に終了・更新",
        "3類型を「補助」へ一元化 ── 必要な範囲のみ権限を付与するオーダーメイド型",
        "後見人の交代を容易に／デジタル遺言の創設（報道）",
    ]
    yy = y + IN(0.42)
    for p in pillars:
        add_rect(s, IN(0.8), yy + IN(0.05), IN(0.13), IN(0.13), GOLD)
        add_text(s, IN(1.05), yy, IN(7.5), IN(0.38), [(p, 11, INK, False)])
        yy += IN(0.4)

    memo_panel(s, IN(9.05), top + IN(0.78), IN(3.48), IN(4.25),
               "当日メモ：施行時期・現場影響")
